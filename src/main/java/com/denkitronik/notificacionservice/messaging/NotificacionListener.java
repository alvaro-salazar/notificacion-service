...
import com.denkitronik.notificacionservice.workers.NotificacionWorker;
...
public class NotificacionListener {
    ...
    private final NotificacionWorker notificacionWorker;
    ...
    public void onEvento(ConsumerRecord<String, String> record) throws Exception {
        log.debug("[CONSUMER] hilo={} topic={} key={} offset={}",
            Thread.currentThread().getName(), record.topic(), record.key(), record.offset());
        ...
        switch (tipo) {
            case "PedidoCreado" -> {
                ...
                log.info("[CONSUMER] hilo={} | PedidoCreado #{} recibido. Delegando al worker.",
                    Thread.currentThread().getName(), p.pedidoId());
                notificacionWorker.procesarPedidoCreado(p);
            }
            case "PedidoActualizado" -> {
                var p = objectMapper.treeToValue(payload, PedidoActualizadoPayload.class);
                log.info("[CONSUMER] hilo={} | PedidoActualizado #{} recibido. Delegando al worker.",
                    Thread.currentThread().getName(), p.pedidoId());
                notificacionWorker.procesarPedidoActualizado(p);
            }
            case "PagoConfirmado" -> {
                var p = objectMapper.treeToValue(payload, PagoConfirmadoPayload.class);
                log.info("[CONSUMER] hilo={} | PagoConfirmado pedido #{} recibido. Delegando al worker.",
                    Thread.currentThread().getName(), p.pedidoId());
                notificacionWorker.procesarPagoConfirmado(p);
            }
            case "PagoRechazado" -> {
                var p = objectMapper.treeToValue(payload, PagoRechazadoPayload.class);
                log.info("[CONSUMER] hilo={} | PagoRechazado pedido #{} recibido. Delegando al worker.",
                    Thread.currentThread().getName(), p.pedidoId());
                notificacionWorker.procesarPagoRechazado(p);
            }
            default ->
                log.warn("[CONSUMER] hilo={} | Tipo de evento desconocido: {} en topic {}",
                    Thread.currentThread().getName(), tipo, record.topic());
        }
    }
    ...
    public void onDlt(ConsumerRecord<String, String> record) {
        ...
        MensajeFallido fallido = new MensajeFallido(
            eventoId, record.topic(), record.value(), errorMensaje, EstadoMensaje.PENDIENTE);
    ...
    private String extractEventoId(String json) {
        try {
            return objectMapper.readTree(json).path("eventoId").asText("sin-id");
        } catch (Exception e) {
            return "sin-id";
        }
    }
